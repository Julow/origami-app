...
let image t ~measure_text =
  ...
  let labels_x =
    List.fold_left (fun acc fold ->
        let text = Printf.sprintf "%.0fmm" fold in
        acc ++ label_left ~measure_text ~rot_line:(Float.pi /. -2.) ~rot_text:(Float.pi /. -4.) (fx +. fold *. x_unit) ~-.fy text
      ) I.void t.x_folds
  in
  let labels_y =
    List.fold_left (fun acc fold ->
        let text = Printf.sprintf "%.0fmm" fold in
        acc ++ label_right ~rot_text:(Float.pi /. -4.) ~-.fx (fy +. fold *. y_unit) text
      ) I.void t.y_folds
    in
...
let ui () =
  ...
  let t =
    let$ w = Lwd.get box_w and$ l = Lwd.get box_l and$ h = Lwd.get box_h in
    let x_folds = [ h; h +. w ] in
    let y_folds = [ h; h +. l ] in
    {
      x_folds;
      y_folds;
      paper_w = int_of_float (w +. (h *. 2.));
      paper_h = int_of_float (l +. (h *. 2.));
    }
  in
  let paper_size_txt =
    let$ t = t in
